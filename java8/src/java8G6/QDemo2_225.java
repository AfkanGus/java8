...
/* queue -очередь */
class Queue {
	private char q[];// масив для хранения элементов очереди
	private int putloc, getloc; // индексы для вcтавки и извлечения элементов очереди
	// Сконструировать пустую очередь заданного размера
	Queue(int size) {
		q = new char[size + 1];// выделить памято для очереди
		putloc = getloc = 0;
	}
...
// Извлечение символов их очереди
	char get() {
		if(getloc == putloc) {
			System.out.println(" -Очередт пуста");
			return (char) 0;
...
public class QDemo2_225 {
	...
	int i;
		...
		// Показать очереди
		System.out.print("Садержимое q1:");
		for(i =0;i<10;i++) {
			ch = q1.get();
			System.out.print(ch);
		}
